...
export type CommandDefinition<Inputs extends CommandInputDefinition = {}> = {
    name: string
...
export const parseInput = (
    ...
    for (let i = 0; i < keyValuePairs.length; i++) {
        ...
        switch (type) {
            ...
            case "num?": {
                ...
                if (isNaN(num)) {
                    return parserError(`"${key}" option is an number${type === "num" || type === "num?" ? "" : " (float)"}, got "${value}"`)
                }
                Object.defineProperty(state, key, {
                    value: num,
                    enumerable: true,
    ...
    for (let i = 0; i < definitionTokens.length; i++) {
        const {name, type} = definitionTokens[i]
        ...
        if (!isOptional && type !== "bool") {
            const friendlyType = ((t: typeof type) => {
                switch (t) {
                    case "float":
                    case "int":
                    case "num":
