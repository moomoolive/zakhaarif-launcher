					...
					if (response.ok) {
						...
						if (
							...
							return fileCache.putFile(
								storageUrl,
								new Response(response.body, responseConfig)
							)
						}
						...
						responseConfig.headers["Content-Length"] = transformedBodyBytes.byteLength.toString()
						return fileCache.putFile(
							storageUrl,
							new Response(transformedBodyBytes, responseConfig)
						)
					}
